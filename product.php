<?php 
	...
	*/
	get_header();
	$GLOBALS['tg_current_page']=$post->post_name;
...
<div class="wrapper_content">
  ...
  <div class="post_type_info">
		<?php
		$gls_unserial=$GLOBALS['gls_products'];
  ...
  <div id="options">
    <ul  id="filters" class="info_filters">
    	<?php	$taxo = 'catalogs'; $results = get_terms($taxo); if(count($results) > 1){ ?>
      <li>
  ...
  <div class="taxonomy_filter">
      ...
      <div style="width:<?php echo $themeshock_gls_box_wdth; ?>" class="taxonomy_item <?php echo $cats?>">
          ...
          <div class='container_hover_shine'>
            <?php 
            	if (has_post_thumbnail( $post->ID )){
            		?>
            		<a href="<?php the_permalink();?>" title="<?php echo $GLOBALS['post_type_options']['caption_img'];?>">
	            		<?php
        ...
        <br />
        <br />
        <?php if($GLOBALS['post_type_options']['item_value']!=""){
			$currency=($GLOBALS['tg_shp_cart']==='true')?$GLOBALS['tg_shp_currency']:$GLOBALS['post_type_options']['currency_val'];					
			echo '<span class="item_value">Price '.$currency.' $'.$GLOBALS['post_type_options']['item_value'].'</span>'; 
			}if($GLOBALS['tg_shp_cart']==='true'){
		?>
		<a style="cursor:pointer;font-family:'<?php echo ($themeshock_gls_btn_fnt_stl)?$themeshock_gls_btn_fnt_stl:'Wire One'; ?>', sans-serif;font-size:<?php echo $themeshock_gls_btn_fnt_sz; ?>;" class="btn shp_cart_button <?php echo ($themeshock_gls_btn_clr)?$themeshock_gls_btn_clr:''; ?>" tg_id_product="<?php the_ID(); ?>" tg_shp_type="tg_shp_add"  onclick="return false;" ondblclick="return false;" href="#">Add to cart</a><!-- añadir el carro del compras -->
        <?php  }else{?>
        <a style="cursor:pointer;font-family:'<?php echo ($themeshock_gls_btn_fnt_stl)?$themeshock_gls_btn_fnt_stl:'Wire One'; ?>', sans-serif;font-size:<?php echo $themeshock_gls_btn_fnt_sz; ?>;" class="btn <?php echo ($themeshock_gls_btn_clr)?$themeshock_gls_btn_clr:''; ?>" href="<?php echo $GLOBALS['post_type_options']['website_url'];?>"><?php echo ($themeshock_gls_btn_txt!='')? $themeshock_gls_btn_txt : 'Buy Now'; ?></a>
        <?php }?>
