...
//			$carouselContainer.find('.carousel-inner').css('background','#000')
 ...
 function func_slider(slidertype,params,bkimage,autoplay,thumb_pos){
	...
	height=(slider_p.fixed)?slider_p.height:parseInt(slider_p.height)-parseInt(jQuery('.slider_area').css('padding-top'));//obtiene el height real reduciondeo el padding
	width=(slider_p.fixed)?slider_p.width:parseInt(slider_p.width)-parseInt(jQuery('.slider_area').css('padding-top'));//obtiene el height real reduciondeo el padding
	switch(thumb_pos){
		case 'top':
		case 'bottom':
